...
<layout:main>
	...
	<jsp:attribute name="javascript">
		...
		$(document).ready(function() {
			...
			$("#btnRemove").click(function() {
				if (confirm(" 삭제하시겠습니까?") == true){
					...
					$.ajax({
						type : "POST",
					    cache: false,						
						url : "/customer/removePopupNotice",
						dataType : "json",
						data : { "pkList" : pkList}
					})
			...
			$("#btnWrite").click(function(){
				location.href = "/customer/popupNoticeRegister?page=" + page + "&searchColumn=" + searchColumn + "&searchString=" + searchString;
			});
		...
		function updateOpenYN(pk, targetCpCode, yn, startTime, endTime) {
				...
				type : "GET",
			    cache: false,						
				url : "/customer/updatePopupNoticeOpenYN",
				dataType : "json",
	...
	<jsp:body>
		...
		<form name="searchForm" method="get" action="/customer/popupNoticeList">
		<div class="boardTitle">
				<div class="borderTitle">
					<span class="pageTitle">팝업공지</span>
		...
		<table class="board mt10">
			<caption>popup공지 조회 결과의 제목, 게시시간, 작성자, 노출여부를 확인 할수 있는 표</caption>
			<colgroup>
				<col style="width:5%" />
				<col style="width:5%" />
				<col style="width:10%" />
			...
			<c:if test="${!empty noticeList}">
				...
				<tr data-pk="${notice.pk}" data-cp-code="${notice.cpCode}" data-start-time="${notice.startTime}" data-end-time="${notice.endTime }" onmouseover="this.style.backgroundColor='#f7f7f7'" onmouseout="this.style.backgroundColor=''" bgcolor="white">
					...
					<td style="text-align:left;">
						&nbsp;<a href="/customer/popupNoticeModify?pk=${notice.pk }&page=${param.page}&searchColumn=${param.searchColumn}&searchString=${param.searchString}&cpCode=${param.cpCode}&openYnSelect=${param.openYnSelect}" onfocus="this.blur();">${notice.title}</a>
					</td>
