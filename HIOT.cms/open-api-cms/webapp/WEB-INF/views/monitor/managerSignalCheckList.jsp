			...
			$(document).ready(function() {
				...
				$("#btnSignalCheck").click(function(){
					if($("#signalCheckListContainer input[type=checkbox]:checked").size() == 0){
						alert("GW 로그 확인 요청할 대상을\n하나 이상 선택 하셔야합니다.");
						return false;
					}
					
					if($("input[name=repeat]:checked").size() == 0) {
						alert("GW 로그 확인 방법을 선택하십시요.");
						return false;
					}
					...
					signalCheckRequest("wifi");
				});	
				
				$("#btnDeviceSignalCheck").click(function(){
					if($("#signalCheckListContainer input[type=checkbox]:checked").size() == 0){
						alert("GW 로그 확인 요청할 대상을\n하나 이상 선택 하셔야합니다.");
						return false;
					}
					...
					signalCheckRequest("device");
				});
			});
			
			function signalCheckRequest(type) {
				var devUUID = "";
				...
				if(type == "wifi") {
				    url = "/monitor/managerSignalCheckRequest";
				}else {
				    url = "/monitor/managerDeviceSignalCheckRequest";
				}
				...
				.done(function(result){
					...
					if(result.code == "200"){
						if(type == "wifi") {
						    alert("홈매니저 허브 로그 확인 요청되었습니다.\n잠시후 '홈매니저 허브 상태정보 조회' 메뉴에서 확인 가능합니다.");
						}else {
						    alert("홈매니저 허브 로그 확인 요청되었습니다.\n잠시후 '홈매니저 IoT 기기 상태정보 결과 조회' 메뉴에서 확인 가능합니다.");
						}
					}else{
						alert("홈매니저 허브 로그 요청이 실패하였습니다.\n잠시후 다시 이용해주세요.");
					}
				})
	...
	<jsp:body>
		...
		<div class="boardTitle">
			<div class="borderTitle">
				<span class="pageTitle">홈매니저 허브 상태정보 확인 요청</span>
			</div>
		</div>
		<div class="boardTop">	
			<div class="flr">
				<form name="searchForm" method="get" action="/monitor/managerSignalCheckList">
		...
		<table style="border-width:1px;border-style:solid;width:100%;height:40px">
			<tr>
				...
				<td style="text-align:center;">
					<input type="button" id="btnSignalCheck" value="WIFI 로그전송 요청" class="btn" style="cursor:pointer">
					<input type="button" id="btnDeviceSignalCheck" value="단말 로그전송 요청" class="btn" style="cursor:pointer">
				</td>
			</tr>
		</table>		
