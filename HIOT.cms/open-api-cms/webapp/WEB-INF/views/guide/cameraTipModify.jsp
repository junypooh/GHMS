...
<layout:main>
	...
	<jsp:attribute name="javascript">
		...
		<script type="text/javascript">
			...
			function viewImage(imagePath) {
				...
				var domain = '${cameraTip.domain}';
				var port = '${cameraTip.port}';
	...
	<jsp:body>
		<form name="transForm" method="get" action="/guide/cameraTipList">
		...
		<c:forEach var="image" items="${cameraTip.imageList }" varStatus="status">
		...
		<table class="tbl_1">
			...
			<tbody>
				<form id="modifyForm" name="modifyForm" method="post" onsubmit="return false;" action="/guide/cameraTipModify" enctype="multipart/form-data">
				<input type="hidden" name="method" value="modify" />
				<input type="hidden" name=pk value="${cameraTip.pk}" />
				<input type="hidden" name="page" value="${param.page }" />
				<input type="hidden" name="searchColumn" value="${param.searchColumn }" />
				<input type="hidden" name="searchString" value="${param.searchString }" />	
				<input type="hidden" name=cpCode value="${param.cpCode}" />
				<input type="hidden" name=targetCpCode value="${cameraTip.cpCode}" />			
				<input type="hidden" name=verNum value="${cameraTip.verNum}" />	
				
				<tr>
					<th scope="row">앱명</th>
					<td>
						<c:choose>
							<c:when test="${cameraTip.cpCode=='001'}">올레기가 홈캠</c:when>
							<c:when test="${cameraTip.cpCode=='002'}">올레기가 홈피트니스</c:when>
							<c:otherwise></c:otherwise>
						</c:choose>
					</td>
					<th scope="row">앱버전</th>
					<td>${cameraTip.verNum}</td>
				</tr>
				<tr>
					<th scope="row">작성자</th>
					<td colspan="3">${cameraTip.writerId}</td>
				</tr>
				<tr>
				...
				<c:if test="${!empty cameraTip.imageList }">
				<tr>
