...
<layout:main>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#btnToggle").click(function(){
					...
					$("#persInfoCollectTermsListContainer input[type=checkbox]").each(function(index){
				...
				$("#btnRemove").click(function() {
					var pkList = "";
					$("#persInfoCollectTermsListContainer tr input[type=checkbox]").each(function(index) {
					...
					if(confirm("정말로 삭제하시겠습니까?") == true) {
						...
						$.ajax({
							type : "POST",
						    cache: false,						
							url : "/terms/removePersInfoCollectTerms",
							dataType : "json",
							data : { "pkList" : pkList}
						})
				...
				$("#btnWrite").click(function(){
					location.href = "/terms/persInfoCollectTermsRegister";
				});
	...
	<jsp:body>
		<form name="searchForm" method="get" action="/terms/persInfoCollectTermsList">
		<div class="boardTitle">
			<div class="borderTitle">
				<span class="pageTitle">개인정보 수집 및 이용동의</span>
		...
		<table class="board mt10">
			<caption>개인정보 수집 및 이용동의에 상품명, 제목, 공고일자, 시행일자, 버전정보를 확인 할수 있는 표</caption>
			<colgroup>
				<col style="width:5%" />
				<col style="width:5%" />
				<col style="width:10%" />
			...
			<tbody id="persInfoCollectTermsListContainer">
			...
			<c:if test="${!empty termsList}">
				...
				<tr data-pk="${terms.pk}" onmouseover="this.style.backgroundColor='#f7f7f7'" onmouseout="this.style.backgroundColor=''" bgcolor="white">
					...
					<td>${terms.cpName }</td>
					<td>
						&nbsp;<a href="/terms/persInfoCollectTermsModify?pk=${terms.pk }&page=${param.page}&cpCode=${param.cpCode}" onfocus="this.blur();">${terms.title}</a>
					</td>
