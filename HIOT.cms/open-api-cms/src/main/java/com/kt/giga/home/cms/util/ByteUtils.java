...
public class ByteUtils 
{
	...
	/**
	 * 바이트 단위 글자 자르기 (한글도 가능)
	 * @param content 자를 글자
	 * @param sByte 자를 바이트(숫자로입력)
	 * @return contents "List&ltString&gt"
	 */
	public static List<String> splitByte(String content, int sByte)
	{
		int count = 0;
		int temi = 0;
		int tCount = sByte < 1 ? 3998 : sByte - 2;
		
		List<String> contents = new ArrayList<String>();
		
		for(int i = 0; i <= content.length(); i++)
		{
			if(count <= tCount && i < content.length())
			{
				count += (content.charAt(i) > 128) ? 2 : 1;
			}
			else
			{
				count = 0;
				contents.add(content.substring(temi,i));
	 ...
	 * XSS 방지 
	 * @param str
	 * @param use_html // html 사용 유무(0:사용안함, 1:부분사용, 2:전부허용)
	 * @return
	 */
	public static String removeXSS(String str, int use_html) {
        ...
        if(use_html<2){
	        ...
	        if(use_html==1){ // HTML tag를 사용하게 할 경우 부분 허용
		        ...
		        str = str.replaceAll("&lt;BR&gt;", "<BR>");
		        str = str.replaceAll("&lt;BR&gt;", "<ol>");
	    		str = str.replaceAll("&lt;BR&gt;", "<li>");
				str = str.replaceAll("&lt;BR&gt;", "<ul>");
				str = str.replaceAll("&lt;BR&gt;", "</ol>"); 
				str = str.replaceAll("&lt;BR&gt;", "</li>"); 
				str = str.replaceAll("&lt;BR&gt;", "</ul>");
				str = str.replaceAll("&lt;BR&gt;", "<OL>"); 
	...
	/**
	 * 스크립트 형식 패턴 지정 (CASE_INSENSITIVE : 대소문자 , MULTILINE : 엔터 , DOTALL : 줄끝문자)
	 */
