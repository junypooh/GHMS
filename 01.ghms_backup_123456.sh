...
backuppath="/jboss/backup/ghms.war."$(date +"%Y%m%d_%H%M%S")

for (( i=1; i<${serverlength}+1; i++));
do
        targetpath="/jboss/applications/api$((i-1))3/ghms.war"
        tput setaf 3
        echo "Start to backup ghms.war at" ${servernames[$i-1]} "- " ${apiservers[$i-1]}
        tput setaf 2
        echo ""
        tput setaf 1
        ssh jboss@${apiservers[$i-1]} "cp $targetpath $backuppath" >/dev/null
